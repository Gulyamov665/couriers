...
import { getAccessToken, isTokenExpired } from "app/tools/tools";
...
let refreshPromise: Promise<QueryReturnValue<unknown, FetchBaseQueryError, FetchBaseQueryMeta>> | null = null;

export const baseQuery = createBaseQuery(baseURL);
export const ordersQuery = createBaseQuery(ordersURL);

export const baseInterceptor: BaseQueryFn<string | FetchArgs, unknown, FetchBaseQueryError> = async (
  args,
  api,
  extraOptions
) => {
  ...
  if (token && isTokenExpired(token.access)) {
    if (!isRefreshing) {
      isRefreshing = true;
      refreshPromise = handleRefreshToken(args, api, extraOptions, baseQuery, token.refresh).finally(() => {
        isRefreshing = false;
        refreshPromise = null;
      });
    }
